...
# --- サイバー・スタイル（ALL-CYBER BLUE + 翻訳ツールチップ） ---
st.markdown("""
    <style>
    /* 全テキストをシアン（水色）に統一 */
...
# 1. ページタイトル（マウスオーバーで翻訳表示）
st.markdown('<h1 class="main-title" title="龍王理論：資産運用ターミナル">DRAGON KING THEORY</h1>', unsafe_allow_html=True)

# --- サイドバー構成 ---
with st.sidebar:
    st.header("🔍 SCAN TARGETS")
    ticker_input = st.text_input("分析銘柄", value="XRP-USD, 7203.T, 3140.T, AAPL", help="分析したい銘柄コードをカンマ区切りで入力。日本株は末尾に .T").upper()
    ...
    for i in range(st.session_state.rows):
        ...
        tick = col1.text_input("銘柄", value="XRP-USD" if i==0 else "", key=f"t_{i}", help="銘柄コード")
        price = col2.number_input("単価", value=0.0, key=f"p_{i}", help="取得単価")
        qty = col3.number_input("数量", value=0.0, key=f"q_{i}", help="保有数量")
        if tick: pf_data_list.append({"銘柄": tick.upper(), "単価": price, "数量": qty})
    
    if st.button("🛰️ ADD UNIT", help="保有銘柄の入力枠を増やします"):
        st.session_state.rows += 1
        st.rerun()

    st.divider()
    st.header("🔔 ALERTS")
    alert_ticker = st.selectbox("対象銘柄", tickers, help="アラートを設定する銘柄を選択")
    target_price = st.number_input("通知価格（以下）", value=0.0, help="この価格を下回ったら通知を発動")
...
st.markdown("<h3 title='艦隊総資産：保有銘柄の合計価値' style='color:#00f2ff; text-align:center;'>🌌 TOTAL ASSET VALUE</h3>", unsafe_allow_html=True)

pf_df, total_cost, total_value = get_live_pf(pf_data_list)
if not pf_df.empty:
    c1, c2, c3 = st.columns([1.5, 1.5, 2])
    c1.metric("TOTAL VALUE", f"¥{total_value:,.0f}" if "T" in ticker_input else f"${total_value:,.2f}", help="現在の総資産評価額")
    c2.metric("TOTAL P/L", f"{(total_value-total_cost):,.2f}", delta=f"{((total_value-total_cost)/total_cost*100):.2f}%", help="取得総額に対する現在の損益額と損益率")
    ...
    st.markdown('<p style="color:#00f2ff; text-align:center; border:1px dashed #00f2ff; padding:20px;">⚠️ SYSTEM READY: PLEASE ENTER FLEET DATA IN SIDEBAR.<br>(サイドバーにポートフォリオを入力してください)</p>', unsafe_allow_html=True)
    ...
    try:
        with st.expander(f"🛰️ SCANNING: {t_code}", expanded=True):
            ...
            if alert_ticker == t_code and target_price > 0 and last_p <= target_price:
                st.balloons()
                st.warning(f"🚨 TARGET REACHED: {t_code} @ {last_p:.2f}")

            # 売買ゾーン表示
            if rsi < 30: st.markdown('<div class="buy-zone" title="売られすぎ。龍が目覚める絶好の買い場です。">🚀 BUY SIGNAL: DRAGON AWAKENING</div>', unsafe_allow_html=True)
            elif rsi > 70: st.markdown('<div class="sell-zone" title="過熱状態。冷静な利確または警戒が必要です。">⚠️ SELL SIGNAL: OVERHEATED</div>', unsafe_allow_html=True)

            # メトリクス（help引数で翻訳表示）
            ca, cb, cc = st.columns(3)
            ca.metric("PRICE", f"{last_p:,.2f}", help="現在の市場価格")
            cb.metric("DIV YIELD", div_text, help="年間配当利回り（予想値）")
            cc.metric("X-DAY", crit_date, help="トレンド転換が起こる可能性が高い推定日（臨界点）")
...
# --- 4. ヘルプセクション ---
st.divider()
with st.expander("❓ OPERATIONAL GUIDE / 操作マニュアル"):
    st.markdown("""
    - **SCAN TARGETS**: 監視したい銘柄を入力。日本株は `コード.T`、米国株はシンボル。
    - **FLEET DATA**: あなたの保有資産。単価と数量を入力すると総評価額をスキャン。
    - **DIV YIELD**: 科学的分析に加え、優待・配当の楽しみも数値化。
    - **X-DAY**: LPPLSモデルによる、トレンドの「終わり」の予測日。
    - **SIGNALS**: RSIに基づき、龍の目覚め（買い）と過熱（売り）を警告。
    """)
